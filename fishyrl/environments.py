...
class VectorizedRLGymEnvironment(VectorizedEnvironment):
    ...
    def __init__(self, num_envs: int = 1, allow_rendering: bool = False, **init_kwargs: dict[str, Any]) -> None:
        ...
        for _ in range(self._num_envs):
            ...
            action_parser = rlact.RepeatAction(
                rlact.LookupTableAction(), repeats=8)
            ...
            reward_fn = rlreward.CombinedReward(
                (rlreward.GoalReward(), 100), (rlreward.TouchReward(), 1))
            # Observations
            ...
            state_mutator = rlstate.MutatorSequence(
                rlstate.FixedTeamSizeMutator(2, 2),
    ...
    def _construct_actions(self, actions: np.ndarray) -> list[dict[str, np.ndarray]]:
        ...
        new_actions = []
        for env, env_actions in zip(self._envs, actions):
            # Get agents
            agents = env.agents
            ...
            for i, agent in enumerate(agents):
                env_new_actions[agent][env_actions[i]] = 1
    ...
    def action_sample(self) -> np.ndarray:
        ...
        :return: An array of sampled actions, with shape (num_envs, num_agents).
        :rtype: np.ndarray
        ...
        for env in self._envs:
            ...
            env_actions = np.random.randint(action_space_size, size=len(env.agents))
    ...
    def step(self, actions: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, dict[str, Any]]:
        ...
        terminations = np.stack(terminations, axis=0)
        truncations = np.stack(truncations, axis=0)
